        ...
        <div className="space-y-3">
          {filtered.map((issue, i) => (
            <motion.div key={issue.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="card p-5">
              ...
              {issue.adminNote && (
                <div className="bg-brand-50 dark:bg-brand-500/10 border border-brand-200 dark:border-brand-500/20 rounded-xl p-3 text-sm text-brand-700 dark:text-brand-300 mb-3">
              ...
              {noteId === issue.id && (
                <div className="mb-3">
                  <textarea value={note} onChange={e => setNote(e.target.value)} rows={2} placeholder="Add admin note..." className="input-field resize-none text-sm mb-2" />
                </div>
              )}

              <div className="flex gap-2 flex-wrap">
                {issue.status === 'Open' && (
                  <button onClick={() => update(issue.id, 'In Progress')} className="flex items-center gap-1 text-xs bg-amber-500 hover:bg-amber-600 text-white px-3 py-1.5 rounded-lg font-semibold transition-all">
                ...
                {issue.status !== 'Resolved' && (
                  <button onClick={() => {
                    if (noteId === issue.id) {
                      update(issue.id, 'Resolved', note)
                      setNoteId(null)
                      setNote('')
                    } else {
                      update(issue.id, 'Resolved')
                    }
                ...
                <button onClick={() => { setNoteId(noteId === issue.id ? null : issue.id); setNote('') }} className="text-xs text-gray-500 hover:text-brand-500 px-2 py-1.5 rounded-lg transition-colors">
                  {noteId === issue.id ? 'Cancel Note' : 'Add Note'}
                </button>
              </div>
            </motion.div>
          ))}
